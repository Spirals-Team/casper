...
public class NullLiteralProcessor extends AbstractProcessor<CtLiteral<?>>{
	...
	public boolean isToBeProcessed(CtLiteral<?> candidate) {
		return candidate.getValue()==null
				&& (
					...
					|| (candidate.getParent() instanceof CtInvocation && candidate.getParent(CtInvocation.class).getExecutable().getDeclaration()!=null)
				);
	}
	...
	private CtTypeReference<?> getType(CtLiteral<?> arg0) {
		...
		if (arg0.getParent() instanceof CtInvocation) {
			CtInvocation<?> invoc = arg0.getParent(CtInvocation.class);
			int indexOfArg = invoc.getArguments().indexOf(arg0);
			return invoc.getExecutable().getDeclaration().getParameters().get(indexOfArg).getType();
		}
		return arg0.getType();
	}
